...
trait LDAOptimizer {
   ...
   */
  private[topicModeling] def initialize(docs: RDD[(Long, Vector)], lda: LDA): LDAOptimizer

  /**
   * run an iteration
   * @return
   */
  private[topicModeling] def next(): LDAOptimizer
   ...
   * @return
   */
  private[topicModeling] def getLDAModel(iterationTimes: Array[Double]): LDAModel
}